...
from openood.networks import ResNet18_32x32, ResNet18_224x224, ResNet50, ResNet34_32x32, ResNet50_224x224
from openood.networks.hamos_net import HamOSNet
...
MODEL = {
    'cifar10': ResNet34_32x32,
    'cifar100': ResNet34_32x32,
    'imagenet200': ResNet50_224x224,
    'imagenet': ResNet50_224x224
}
...
for subfolder in sorted(glob(os.path.join(root, 's*'))):
    ...
    pre_postprocessor_name = postprocessor_name

    # load the pretrained model provided by the user
    if postprocessor_name == 'hamos':
        backbone = model_arch(num_classes=num_classes)
        net = HamOSNet(backbone,
                      head='mlp',
                      feat_dim=args.feat_dim,
    ...
    net.load_state_dict(torch.load(os.path.join(subfolder, 'best.ckpt'), map_location='cpu'))
    net.cuda()
    net.eval()
    ...
    postprocessor_name = pre_postprocessor_name
